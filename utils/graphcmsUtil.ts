...
export const mapProductToProps: (product: RawProject) => Product = ({ description, ...product }) => ({
  ...product,
...
export const mapProductsToProps: (
  ...
  thumbnails?: RawThumbnail[]
    ...
    products.map((product: RawProject) => ({
      ...
      ...(thumbnails ? { thumbnail: thumbnails.find((thumb) => thumb.slug === product.slug)?.thumbnail } : {})
    }))

export const mapSeoToProps = ({ pages, seoCommons }: { pages: RawSeoPage[], seoCommons: RawSeoCommons[] }): MappedSeoProps => ({
